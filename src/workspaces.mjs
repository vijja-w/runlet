...
export function getActionTemplate(kind = 'script', withView = false) {
  ...
  return {
    kind,
    ...shared,
    runJsGuidance: 'run.js must export one default async function receiving { workspace, run }. Use only workspace.read/write/list/exists/mkdir/delete/fetch and run.log. Do not import modules or access paths outside the registered workspace.',
  };
}
...
export function getScriptTemplate(withView = false) {
  return {
    ...
    rules: [
      ...
      'run.js receives { workspace, run, input }. Use input values declared by the controls.',
      ...
      'Use only workspace.read/write/list/exists/mkdir/delete/fetch and run.log.',
